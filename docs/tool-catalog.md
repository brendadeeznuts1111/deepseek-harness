...
| `@deepseek-ai/dsh-tool-bun` | `bun` | `ctx.tools`, `ctx.bun`, `ctx.systemPrompt`, `ctx.shellEnv`, `ctx.jobs at call time for run_in_background` | `tool/call`, `tool/result` | - | The bun tool is the model-facing consumer of the opt-in Bun executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@deepseek-ai/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled. `args` is argv after the resolved bun binary, not a shell string. |
...
Source: [`packages/bun/tool-bun/src/index.ts`](../packages/bun/tool-bun/src/index.ts)
